...
\section{Manejo de Archivos Pesados (BLOBs y Multimedia)}
Para el soporte de archivos pesados de multimedia (carátulas de juegos y tráilers de video en formato MP4), la arquitectura implementa dos modalidades de almacenamiento configurables por la empresa al momento de subir o editar su juego.

\subsection{Carga de Imágenes y Videos (Upload en Modo BLOB)}
Cuando se selecciona la modalidad de almacenamiento binario puro en la base de datos (BLOB), Flask recibe los archivos binarios del cliente en memoria y los inyecta en el motor de Oracle mediante variables seguras (\textit{Bind Variables}) para evitar fragmentaciones.

\begin{lstlisting}[caption=games\_controller.py (Carga de BLOBs)]
...
sql = """INSERT INTO videojuego (id_empresa, titulo, portada, trailer, modo_almacenamiento) 
         VALUES (:empresa, :titulo, :portada, :trailer, 'BLOB')"""
\end{lstlisting}

\subsection{Visualización y Transmisión (Streaming)}
Dado que el video se almacena directamente en formato binario dentro de Oracle, no es posible apuntar a una ruta estática en disco. El middleware en Flask intercepta las peticiones de los reproductores multimedia, lee el segmento binario del registro del juego y lo transmite como un flujo binario en tiempo real con soporte para cabeceras de rango (\textit{Range Requests}) y tipo MIME.

\begin{lstlisting}[caption=games\_controller.py (Servicio de BLOB y Streaming)]
...
\subsection{Modalidad Híbrida: Archivos en Disco (Modo RUTA)}
Cuando una empresa prefiere optimizar el espacio de almacenamiento del motor de base de datos, puede configurar el juego en modo \texttt{RUTA}. En esta modalidad, Flask se encarga de guardar físicamente los archivos en el servidor bajo la estructura \texttt{static/uploads/portadas/} y \texttt{static/uploads/trailers/}, mientras que la base de datos solo almacena las cadenas con las rutas relativas en las columnas \texttt{portada\_ruta} y \texttt{trailer\_ruta}. El controlador decide dinámicamente cómo despachar la información dependiendo del valor de la columna \texttt{modo\_almacenamiento}.

\begin{lstlisting}[caption=games\_controller.py (Almacenamiento en Disco)]
# Guardado fisico en el servidor de Flask
...
\section{Comandos SQL*Plus y Diccionario de Datos para Auditoría}
Para poder verificar la integridad de la arquitectura física y los objetos construidos en Oracle Database desde la consola, el administrador puede utilizar comandos específicos dirigidos al Diccionario de Datos.

\subsection{Ingreso al Sistema como Administrador}
Para conectarse directamente a la base de datos con los privilegios de administrador de base de datos (DBA) y evaluar el esquema, se ejecuta:
\begin{lstlisting}[language=bash]
# Ingresar a Oracle como Administrador Supremo (SYSDBA)
sqlplus sys/AdminP123@FREEPDB1 as sysdba
\end{lstlisting}

\subsection{Consultas Administrativas al Diccionario de Datos}
Las siguientes consultas permiten auditar el estado físico de los segmentos de almacenamiento, tablespaces, disparadores y objetos de la base de datos:

\begin{lstlisting}[caption=Consultas de Auditoria de DBA]
-- 1. Listar las tablas creadas, su tablespace fisico y su PCTFREE
SELECT table_name, tablespace_name, pct_free 
FROM dba_tables 
WHERE owner = 'PROYECTO_BDA' 
ORDER BY table_name;

-- 2. Validar el estado y evento de los disparadores activos en el esquema
SELECT trigger_name, trigger_type, table_name, status 
FROM dba_triggers 
WHERE table_owner = 'PROYECTO_BDA';

-- 3. Auditar los segmentos de almacenamiento asignados a datos LOB (BLOB)
SELECT table_name, column_name, segment_name, tablespace_name 
FROM dba_lobs 
WHERE owner = 'PROYECTO_BDA' AND table_name = 'VIDEOJUEGO';
\end{lstlisting}
